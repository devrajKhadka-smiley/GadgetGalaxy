...
<title>View Order | Gadget Galaxy</title>
<style type="text/css">
* {
	margin: 0;
	padding: 0;
...
table.product-display td form {
	display: flex;
	justify-content: center;
	align-items: center;
}

table.product-display td form button {
	border: none;
	outline: none;
	font-size: 20px;
...
table.product-display td form button:hover {
					...
					<tbody>
						<c:forEach var="order" items="${orderList }">
							...
							<tr>
								...
								<td><c:out value="${order.clientUsername}"></c:out></td>
								<td><c:out value="${order.orderDate}"></c:out></td>
								<td><c:out value="${order.orderStatus }"></c:out></td>
								<td id="no-padding-td">
									<form action="change-order-status" method="get">
										<input type="text" name="orderId" value="${order.orderId }"
											style="display: none;">
										<button class="deliver" type="submit">Deliver</button>
									</form>
								</td>
							</tr>
						</c:forEach>
